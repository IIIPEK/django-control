...
class Command(BaseCommand):
    help = 'Synchronize the managed parameter catalog and import non-secret values.'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            required=True,
            type=Path,
            help='Path to the source environment file.',
        )
        parser.add_argument(
            '--environment',
            required=True,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'Managed catalog sync: {mode}'))
        self.stdout.write(
        ...
        self._write_key_list(
            'Keys not registered in the managed catalog',
            unknown_keys,
        )
