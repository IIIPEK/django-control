...
PARAMETERS = (
    ...
    _spec('TEAMS_TRANSCRIPT_FOCUS_USER_ID', 'teams-transcripts', 'teams-transcript-worker', 'Focus user ID', 'Entra Object ID центрального участника, присутствие которого обязательно для обработки разговора.', required=True, restart=False, order=40),
    _spec('TEAMS_TRANSCRIPT_PRIORITY_CONTACT_USER_ID', 'teams-transcripts', 'teams-transcript-worker', 'Priority contact user ID', 'Entra Object ID контакта с отдельным правилом классификации и маршрутизации.', required=True, restart=False, order=50),
    ...
    _spec('TEAMS_TRANSCRIPT_SOURCE_ROOT_PATH', 'teams-transcripts', 'teams-transcript-worker', 'Source transcripts root', 'Корневая папка исходных транскрипций внутри библиотеки SharePoint.', default='Source transcripts', rules={'min_length': 1}, restart=False, order=90),
    _spec('TEAMS_TRANSCRIPT_SALES_PATH', 'teams-transcripts', 'teams-transcript-worker', 'Sales folder', 'Папка транскрипций разговоров центрального участника с Sales.', default='Sales', rules={'min_length': 1}, restart=False, order=100),
    _spec('TEAMS_TRANSCRIPT_PRIORITY_CONTACT_PATH', 'teams-transcripts', 'teams-transcript-worker', 'Priority contact folder', 'Папка транскрипций разговоров центрального участника с приоритетным контактом.', default='Priority contact', rules={'min_length': 1}, restart=False, order=110),
    _spec('TEAMS_TRANSCRIPT_GROUP_CALLS_PATH', 'teams-transcripts', 'teams-transcript-worker', 'Group calls folder', 'Папка групповых звонков с центральным участником.', default='Group calls', rules={'min_length': 1}, restart=False, order=120),
    _spec('TEAMS_TRANSCRIPT_NOTIFICATION_URL', 'teams-transcripts', 'teams-transcript-worker', 'Notification URL', 'Полный публичный HTTPS URL endpoint для Microsoft Graph change notifications.', data_type='url', required=True, order=130),
    ...
    _spec('TEAMS_TRANSCRIPT_PROCESSING_STALE_SECONDS', 'teams-transcripts', 'teams-transcript-worker', 'Processing stale timeout', 'Через сколько секунд незавершённая обработка считается зависшей и может быть восстановлена.', data_type='integer', default=1800, rules={'min': 60, 'max': 86400}, restart=False, order=215),
    ...
    _spec('TEAMS_TRANSCRIPT_SUMMARY_ENABLE_THINKING', 'teams-transcripts', 'teams-transcript-worker', 'Enable summary thinking', 'Разрешить режим thinking при создании LLM-отчётов.', data_type='boolean', default=False, restart=False, order=225),
